	...
	if ( ! empty( $page_title ) && $wishlist && $wishlist->has_items() ) :
		...
		<div class="wishlist-title mb-4 <?php echo esc_attr( $is_custom_list ) ? 'wishlist-title-with-form' : ''; ?>">
			...
			<?php if ( $is_custom_list ) : ?>
				<a class="btn button show-title-form btn-rounded btn-dark btn-sm btn-icon-left" role="button">
					<?php echo apply_filters( 'yith_wcwl_edit_title_icon', '<i class="w-icon-pencil"></i>' ); ?>
		...
		<?php if ( $is_custom_list ) : ?>
			<div class="hidden-title-form mb-4">
				...
				<a class="hide-title-form btn button btn-sm btn-dark btn-rounded btn-icon-left" role="button">
					<?php echo apply_filters( 'yith_wcwl_cancel_wishlist_title_icon', '<i class="w-icon-undo"></i>' ); ?>
