...
<table class="shop_table cart wishlist_table wishlist_view traditional responsive" data-pagination="<?php echo esc_attr( $pagination ); ?>" data-per-page="<?php echo esc_attr( $per_page ); ?>" data-page="<?php echo esc_attr( $current_page ); ?>" data-id="<?php echo esc_attr( $wishlist_id ); ?>" data-token="<?php echo esc_attr( $wishlist_token ); ?>">
	...
	<thead class="<?php echo false != $wishlist && $wishlist->has_items() ? '' : 'd-none'; ?>">
	<tr>
		<?php
		if ( $show_cb ) :
			++ $column_count;
		...
		<th class="product-thumbnail"><?php esc_html_e( 'Product', 'wolmart' ); ?></th>
		...
		if ( $show_quantity ) :
			...
			<th class="product-quantity">
				<span class="nobr">
					<?php echo apply_filters( 'yith_wcwl_wishlist_view_quantity_heading', esc_html__( 'Quantity', 'yith-woocommerce-wishlist' ) ); ?>
				</span>
			</th>
		<?php endif; ?>
		...
		if ( $show_last_column ) :
			++ $column_count;
			?>
			<th class="product-add-to-cart"><?php esc_html_e( 'Actions', 'wolmart' ); ?></th>
		<?php endif; ?>
		...
		if ( $enable_drag_n_drop ) :
			...
			<th class="product-arrange">
				<span class="nobr">
					<?php echo apply_filters( 'yith_wcwl_wishlist_view_arrange_heading', esc_html__( 'Arrange', 'yith-woocommerce-wishlist' ) ); ?>
				</span>
			</th>
		<?php endif; ?>
	</tr>
	...
	<tbody class="wishlist-items-wrapper<?php echo false != $wishlist && $wishlist->has_items() ? '' : ' wishlist-empty'; ?>">
	<?php
	if ( $wishlist && $wishlist->has_items() ) :
				...
				<tr id="yith-wcwl-row-<?php echo esc_attr( $item->get_product_id() ); ?>" data-row-id="<?php echo esc_attr( $item->get_product_id() ); ?>">
					...
					<td class="product-thumbnail">
						...
						<?php if ( $show_remove_product ) : ?>
							<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ); ?>" class="remove remove_from_wishlist w-icon-times-solid" title="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', esc_html__( 'Remove this product', 'yith-woocommerce-wishlist' ) ); ?>" aria-label="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', esc_html__( 'Remove this product', 'yith-woocommerce-wishlist' ) ); ?>" role="button"></a>
						<?php endif; ?>
					</td>
					...
					<?php if ( $show_price || $show_price_variations ) : ?>
						<td class="product-price">
							<?php
							if ( $show_price ) {
								echo wolmart_strip_script_tags( $item->get_formatted_product_price() );
							}

							if ( $show_price_variations ) {
								echo wolmart_strip_script_tags( $item->get_price_variation() );
							}
							?>
						</td>
					<?php endif ?>
					...
					<?php if ( $show_quantity ) : ?>
						<td class="product-quantity">
							...
							<?php else : ?>
								<?php echo wolmart_strip_script_tags( $item->get_quantity() ); ?>
							<?php endif; ?>
						</td>
					<?php endif; ?>
					...
					<?php if ( $show_stock_status ) : ?>
						<td class="product-stock-status">
							<?php echo 'out-of-stock' == $stock_status ? '<span class="wishlist-out-of-stock">' . apply_filters( 'yith_wcwl_out_of_stock_label', esc_html__( 'Out of stock', 'yith-woocommerce-wishlist' ) ) . '</span>' : '<span class="wishlist-in-stock">' . apply_filters( 'yith_wcwl_in_stock_label', esc_html__( 'In Stock', 'yith-woocommerce-wishlist' ) ) . '</span>'; ?>
						</td>
					<?php endif ?>
						...
						<td class="product-add-to-cart">
							<div class="d-lg-flex">
								...
								if ( $show_dateadded && $item->get_date_added() ) :
									echo '<span class="dateadded">' . sprintf( esc_html__( 'Added on: %s', 'yith-woocommerce-wishlist' ), $item->get_date_added_formatted() ) . '</span>';
								endif;
								?>
								...
								if ( ! is_array( $show_info ) || in_array( 'quickview', $show_info ) ) {
									echo '<button class="btn btn-quickview btn-outline btn-default btn-rounded btn-sm" data-product="' . $product->get_id() . '" title="' . esc_html__( 'Quick View', 'wolmart' ) . '">' . esc_html__( 'Quick View', 'wolmart' ) . '</button>';
								}
								?>
								...
								<?php if ( $move_to_another_wishlist && $available_multi_wishlist && count( $users_wishlists ) > 1 ) : ?>
									...
									<?php else : ?>
										<a href="#move_to_another_wishlist" class="move-to-another-wishlist-button" data-rel="prettyPhoto[move_to_another_wishlist]" role="button">
											<?php echo apply_filters( 'yith_wcwl_move_to_another_list_label', esc_html__( 'Move to another list &rsaquo;', 'yith-woocommerce-wishlist' ) ); ?>
										</a>
									<?php endif; ?>
								<?php endif; ?>
							</div>
		...
		?>
		<tr>
			<td colspan="<?php echo esc_attr( $column_count ); ?>"><i class="w-icon-heart3 empty-icon"></i></td>
		</tr>
		<tr>
			<td colspan="<?php echo esc_attr( $column_count ); ?>"><?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', esc_html__( 'No products added to the wishlist', 'yith-woocommerce-wishlist' ) ); ?></td>
		</tr>
		<tr>
			<td colspan="<?php echo esc_attr( $column_count ); ?>">
				<a class="woocommerce-Button button" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
					<?php esc_html_e( 'GO SHOP', 'wolmart' ); ?>
				</a>
			</td>
		</tr>
		<?php
	...
	if ( ! empty( $page_links ) ) :
		...
		<tr class="pagination-row">
			<td colspan="<?php echo esc_attr( $column_count ); ?>"><?php echo wolmart_strip_script_tags( $page_links ); ?></td>
		</tr>
	<?php endif ?>
	</tbody>
