 ...
 */
if ( ! class_exists( 'WOLMART_WXR_Parser' ) ) {
	class WOLMART_WXR_Parser {
		function parse( $file ) {
			// Attempt to use proper XML parsers first
			if ( extension_loaded( 'simplexml' ) ) {
				$parser = new WOLMART_WXR_Parser_SimpleXML;
				...
				$parser = new WOLMART_WXR_Parser_XML;
				$result = $parser->parse( $file );
			...
			if ( isset( $result ) && defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
				...
				echo '</pre>';
				echo '<p><strong>' . esc_html__( 'There was an error when reading this WXR file', 'wolmart' ) . '</strong><br />';
				esc_html_e( 'Details are shown above. The importer will now try again with a different parser...', 'wolmart' ) . '</p>';
			}

			// use regular expressions if nothing else available or this is bad XML
			$parser = new WOLMART_WXR_Parser_Regex;
 ...
 */
if ( ! class_exists( 'WOLMART_WXR_Parser_SimpleXML' ) ) {
	class WOLMART_WXR_Parser_SimpleXML {
		function parse( $file ) {
			...
			if ( ! $success || isset( $dom->doctype ) ) {
				return new WP_Error( 'SimpleXML_parse_error', esc_html__( 'There was an error when reading this WXR file', 'wolmart' ), libxml_get_errors() );
			}
			...
			if ( ! $wxr_version ) {
				return new WP_Error( 'WOLMART_WXR_parse_error', esc_html__( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'wolmart' ) );
			}
 ...
 */
if ( ! class_exists( 'WOLMART_WXR_Parser_XML' ) ) {
	#[\AllowDynamicProperties]
	class WOLMART_WXR_Parser_XML {
		var $wp_tags     = array(
			'wp:post_id',
			'wp:post_date',
			'wp:post_date_gmt',
 ...
 */
if ( ! class_exists( 'WOLMART_WXR_Parser_Regex' ) ) {
	class WOLMART_WXR_Parser_Regex {
		...
		function parse( $file ) {
			...
			if ( ! $wxr_version ) {
				return new WP_Error( 'WOLMART_WXR_parse_error', __( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'wolmart' ) );
			}
