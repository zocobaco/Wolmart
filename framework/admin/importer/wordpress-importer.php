...
require WOLMART_ADMIN . '/importer/parsers.php';
...
if ( class_exists( 'WP_Importer' ) ) {
	class WP_Import extends WP_Importer {
		...
		function import_end() {
			...
			echo '<p>' . esc_html__( 'All done.', 'wolmart' ) . ' <a href="' . esc_url( admin_url() ) . '">' . esc_html__( 'Have fun!', 'wolmart' ) . '</a>' . '</p>';
			echo '<p>' . esc_html__( 'Remember to update the passwords and roles of imported users.', 'wolmart' ) . '</p>';
		...
		function handle_upload() {
			...
			if ( isset( $file['error'] ) ) {
				...
				printf( wolmart_strip_script_tags( esc_html__( 'The export file could not be found at %s. It is likely that this was caused by a permissions problem.', 'wolmart' ) ), esc_html( $file['file'] ) );
				echo '</p>';
				return false;
			}
...
<?php endif; ?>
	...
	<p class="submit"><input type="submit" class="button" value="<?php esc_attr_e( 'Submit', 'wolmart' ); ?>" /></p>
			...
			foreach ( $this->posts as $post ) {
				...
				*/
				...
				if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
					...
					printf( esc_html__( '%1$s &#8220;%2$s&#8221; already exists.', 'wolmart' ), $post_type_object->labels->singular_name, esc_html( $post['post_title'] ) );
					echo '<br />';
					...
					if ( in_array( $post['post_type'], Wolmart_Setup_Wizard::get_instance()->demo_import_post_types ) ) {
						...
						$meta_fields = array( '_wpb_post_custom_css', '_wpb_shortcodes_custom_css', '_elementor_data', '_elementor_css', '_elementor_edit_mode', '_elementor_page_settings', '_elementor_version', 'page_js', 'page_css', '_elementor_page_assets' );
		...
		function process_attachment( $post, $url ) {
			...
			do_action( 'wolmart_importer_insert_attachment', $post_id, isset( $post['import_id'] ) ? (int) $post['import_id'] : false );
