 ...
 * @version   2.6.1 for parent theme Wolmart Admin for publication on ThemeForest
...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		 ...
		 * @since 2.2.0
		 *
		 * @var string Message filtered by wolmart_strip_script_tags(). Default is empty string.
		 */
		public $message = '';
		...
		public function install_plugins_page() {
			...
			<div class="tgmpa wrap">
				...
				if ( ! empty( $this->message ) && is_string( $this->message ) ) {
					echo wolmart_strip_script_tags( $this->message );
				}
				?>
				<?php $plugin_table->views(); ?>

				<form id="tgmpa-plugins" method="post">
		...
		protected function activate_single_plugin( $file_path, $slug, $automatic = false ) {
			if ( $this->can_plugin_activate( $slug ) ) {
				...
				if ( is_wp_error( $activate ) ) {
					echo '<div id="message" class="error"><p>', wolmart_strip_script_tags( $activate->get_error_message() ), '</p></div>',
			...
			if ( ! empty( $message ) || $total_required_action_count > 0 ) {
				...
				if ( ! current_user_can( 'activate_plugins' ) && ! current_user_can( 'install_plugins' ) && ! current_user_can( 'update_plugins' ) ) {
					...
					if ( ! $this->dismissable && ! empty( $this->dismiss_msg ) ) {
						$rendered .= sprintf( $line_template, wolmart_strip_script_tags( $this->dismiss_msg ) );
					}
...
if ( ! class_exists( 'TGMPA_Utils' ) ) {
	...
	class TGMPA_Utils {
		...
		public static function wrap_in_em( $string ) {
			return '<em>' . wolmart_strip_script_tags( $string ) . '</em>';
		}
		...
		public static function wrap_in_strong( $string ) {
			return '<strong>' . wolmart_strip_script_tags( $string ) . '</strong>';
		}
