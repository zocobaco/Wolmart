...
<div data-taxonomy="<?php echo esc_attr( $attribute->get_taxonomy() ); ?>" class="wcmp-metabox-wrapper woocommerce_attribute wc-metabox closed <?php echo esc_attr( implode( ' ', $metabox_class ) ); ?>" rel="<?php echo esc_attr( $attribute->get_position() ); ?>">
	<div class="wcmp-metabox-title variation-title" data-toggle="collapse" data-target="#attribute_<?php echo esc_attr( $i ); ?>"  aria-expanded="false" aria-controls="collapseExample">
		...
		<div class="wcmp-metabox-action variation-action">
			...
			<a href="#" class="remove_row delete remove-attribute" role="button"><?php esc_html_e( 'Remove', 'dc-woocommerce-multi-vendor' ); ?></a>
		</div>
	</div>
				...
				<tr>
					...
					<td rowspan="3" width="65%">
						...
						if ( $attribute->is_taxonomy() && $attribute_taxonomy = $attribute->get_taxonomy_object() ) {
							...
							?>
							<textarea class="form-control" name="wc_attributes[<?php echo esc_attr( $i ); ?>][value]" cols="5" rows="5" placeholder="<?php printf( esc_attr__( 'Enter some text, or some attributes by "%s" separating values.', 'dc-woocommerce-multi-vendor' ), WC_DELIMITER ); ?>"><?php echo esc_textarea( wc_implode_text_attributes( $attribute->get_options() ) ); ?></textarea>
																						  <?php
						}
						?>
					</td>
