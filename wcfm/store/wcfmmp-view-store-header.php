		...
		<div class="header_area">
			<div class="lft header_left">
				<div class="personal-info">
					...
					<div class="logo-area"><a href="#" aria-label="<?php esc_attr_e( 'Avatar', 'wolmart' ); ?>"><img src="<?php echo esc_url( $gravatar ); ?>" alt="<?php esc_attr_e( 'Logo', 'wolmart' ); ?>"/></a></div>
					...
					<div class="rating-wrap">
						...
						if ( apply_filters( 'wcfm_is_pref_vendor_reviews', true ) && apply_filters( 'wcfm_is_allow_review_rating', true ) ) {
							...
							$vendor_rating = intval( $WCFMmp->wcfmmp_reviews->get_vendor_review_rating( $store_user->get_id() ) );

							echo wc_get_rating_html( $vendor_rating );
						}
						?>
				...
				<div class="address-info flex-1">
					<div class="address rgt">
						<?php if ( ( $WCFMmp->wcfmmp_vendor->get_vendor_name_position( $store_user->get_id() ) == 'on_header' ) && apply_filters( 'wcfm_is_allow_store_name_on_header', true ) ) { ?>
						...
						<?php if ( $address && ( 'no' == $store_info['store_hide_address'] ) && wcfm_vendor_has_capability( $store_user->get_id(), 'vendor_address' ) ) { ?>
							<p class="<?php echo esc_attr( $store_address_info_class ); ?> wcfmmp_store_header_address">
								...
								if ( apply_filters( 'wcfmmp_is_allow_address_map_linked', true ) ) {
									...
									?>
									<a href="<?php echo esc_url( $map_search_link ); ?>" target="_blank"><span><?php echo esc_html( $address ); ?></span></a>
										<?php } else { ?>
						...
						<div class="<?php echo esc_attr( $store_address_info_class ); ?>">
							...
							<?php if ( $phone && 'no' == ( $store_info['store_hide_phone'] ) && wcfm_vendor_has_capability( $store_user->get_id(), 'vendor_phone' ) ) { ?>
								<div class="store_info_parallal wcfmmp_store_header_phone" style="margin-right: 10px;">
									...
									<span>
										...
										<a href="tel:<?php echo esc_attr( $phone ); ?>" role="button"><?php echo esc_html( $phone ); ?></a>
										<?php } else { ?>
							...
							<?php if ( $email && 'no' == ( $store_info['store_hide_email'] ) && wcfm_vendor_has_capability( $store_user->get_id(), 'vendor_email' ) ) { ?>
								<div class="store_info_parallal wcfmmp_store_header_email">
									...
									<span>
										...
										<a href="mailto:<?php echo apply_filters( 'wcfmmp_mailto_email', $email, $store_user->get_id() ); ?>" role="button"><?php echo esc_html( $email ); ?></a>
										<?php } else { ?>
