...
foreach ( $wcfm_store_hours_day_times as $wcfm_store_hours_day => $wcfm_store_hours_day_time_slots ) {
	if ( in_array( $wcfm_store_hours_day, $wcfm_store_hours_off_days ) ) {
		continue;
	}
	...
	echo '<p class="wcfmmp-store-hours-item">';
	...
	foreach ( $wcfm_store_hours_day_time_slots as $slot => $wcfm_store_hours_day_time_slot ) {
		echo date_i18n( wc_time_format(), strtotime( $wcfm_store_hours_day_time_slot['start'] ) ) . ' - ' . date_i18n( wc_time_format(), strtotime( $wcfm_store_hours_day_time_slot['end'] ) );
	}
	echo '</p>';
}
