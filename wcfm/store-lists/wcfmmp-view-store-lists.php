...
<div id="wcfmmp-stores-lists" class="wcfmmp-stores-listing <?php echo esc_attr( $wcfm_store_lists_wrapper_class ); ?>">
	...
	<?php
	if ( $map && apply_filters( 'wcfmmp_is_allow_map_full_width', true ) ) {
		$WCFMmp->template->get_template( 'store-lists/wcfmmp-view-store-lists-map.php', $args ); }
	?>
  
	<?php do_action( 'wcfmmp_store_lists_after_map' ); ?>

	<?php
	if ( ( ! $WCFMmp->wcfmmp_vendor->is_store_lists_sidebar() || ! $sidebar ) && $filter && ( $search || $category || $radius || $country || $state || $has_city || $has_zip ) ) {
		$WCFMmp->template->get_template( 'store-lists/wcfmmp-view-store-lists-search-form.php', $args ); }
	?>
	...
	<?php do_action( 'wcfmmp_store_lists_before_loop' ); ?>
  	...
  	<div id="wcfmmp-stores-wrap-holder">
